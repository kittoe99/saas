...
export default function AITestPage() {
  ...
  return (
    <div className="ai-page mx-auto max-w-none sm:max-w-3xl px-2 sm:px-4 py-8">
      {/* Branded header */}
      <div className="mb-6">
        <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium bg-neutral-900 text-white shadow-sm">
          <span>AI</span>
          <span className="opacity-80">Assistant</span>
        </div>
        <h1 className="text-3xl font-semibold mt-3 tracking-tight">Ask. Search. Create.</h1>
        <p className="text-sm text-neutral-600 mt-1">{providerHint}</p>
      </div>
          ...
          {messages.length === 0 ? (
            ...
            messages.map((m, idx) => (
              <div key={idx} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`${m.role === "assistant" ? "max-w-[98%] sm:max-w-[85%]" : "max-w-[95%] sm:max-w-[85%]"} rounded-2xl px-3.5 py-2.5 shadow-sm animate-fadeInUp ${m.role === "user" ? "bg-neutral-900 text-white" : "bg-gray-50 text-gray-900 dark:bg-neutral-800 dark:text-gray-100"}`}>
                  {m.role === "assistant" ? (
                    <div className="text-[15px] leading-relaxed">
                      ...
                      {m.searchInfo && (
                        <div className="mb-3">
                          <div className="text-xs font-semibold uppercase tracking-wide text-neutral-800 dark:text-neutral-200">Searching Web</div>
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          a: (props) => <a className="underline text-neutral-900 hover:opacity-80" target="_blank" rel="noreferrer" {...props} />,
